// original: https://www.npmjs.com/package/youtube-search-api
...
const GetYoutubeInitData = async (url) => {
  ...
  try {
    const page = await httpClient.get(encodeURI(url), { responseType: 'text' });
...
const nextPage = async (nextPage, withPlaylist = false, limit = 0) => {
  ...
  try {
    ...
    const page = await httpClient.post(
      encodeURI(endpoint),
      { 
...
export const GetListByKeyword = GetData
